...
- 屏幕标注：在主屏源窗口上绘制画笔、荧光笔、直线、箭头、矩形和椭圆，副屏同步显示，默认 `Ctrl + Alt + Shift + A`。

放大倍数、指针效果范围和四组快捷键均可在控制面板中调整。屏幕放大与指针放大镜互斥，聚光灯可以与任一模式组合。标注工具栏支持撤销、重做、清空和橡皮擦；按 `Esc` 可退出标注。标注内容仅在本次镜像期间保留。
